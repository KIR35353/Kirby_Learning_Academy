...
// GET /api/admin/tenants/[id]/departments
export async function GET(_req: NextRequest, { params }: Params) {
  ...
  return NextResponse.json({
    departments: rows.map((r) => ({ id: r.id, name: r.name, userCount: r._count.users })),
  });
}

const createSchema = z.object({ name: z.string().min(1) });

// POST /api/admin/tenants/[id]/departments
export async function POST(req: NextRequest, { params }: Params) {
  ...
  try {
    ...
    return NextResponse.json({ department: { ...department, userCount: 0 } }, { status: 201 });
  } catch {
